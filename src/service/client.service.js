...
const allClient = ({  }) => {
    return db.query("SELECT first_name,last_name, email, birthdate, telephone, id_user," +
            "user_name, pass, rol FROM client C, users U WHERE C.id_user = U.id;");
};

const findOne = ({ id }) => {
    return db.query("SELECT first_name,last_name, email, birthdate, telephone, id_user,"+
     "user_name, pass FROM client C, users U WHERE C.eliminate = TRUE AND U.eliminate = TRUE AND C.id_user = U.id;", [+id]);
};
...
const deleteC =  ({ id }) => {
    return db.query("UPDATE client SET eliminate=true WHERE id_user=$1 RETURNING *", [+id]);
}
